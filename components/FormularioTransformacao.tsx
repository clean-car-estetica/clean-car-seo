  ...
  return (
    <form action={handleSubmit} className="grid gap-4">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <ImageUploader name="imagem_antes_upload" initialUrl={item.imagem_antes} label="Foto de ANTES" aspect={4 / 3} specTexto="1000x750px (proporção 4:3)" onUrlChange={setImagemAntes} nomeArquivo={`${item.titulo} antes`} />
        <ImageUploader name="imagem_depois_upload" initialUrl={item.imagem_depois} label="Foto de DEPOIS" aspect={4 / 3} specTexto="1000x750px (proporção 4:3)" onUrlChange={setImagemDepois} nomeArquivo={`${item.titulo} depois`} />
      </div>
